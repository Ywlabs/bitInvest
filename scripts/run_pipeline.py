...
def main() -> int:
    ...
    with JobLogger("pipeline") as log:
        ...
        if state.errors:
            ...
            log.set_summary("파이프라인 실패: " + "; ".join(state.errors))
            log.set_exit_code(1)
            return 1
        ...
        action = decision.get("action", "-") if not state.skipped else "SKIP"
        log.set_summary(
            f"스냅샷 {state.snapshot_id}, 신호 {state.signal_id}, 매매 {action}"
        )
        return 0
